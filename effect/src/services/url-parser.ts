...
// Service tag
export class UrlParser extends Context.Tag("UrlParser")<
  UrlParser,
  UrlParserService
>() {}
...
const parseBskyApp = (url: URL): ParsedInput | null => {
  ...
  return identifier.startsWith("did:") 
    ? { kind: "did", value: identifier }
    : { kind: "handle", value: identifier }
}
...
export const UrlParserLive: UrlParserService = {
  parseUrl: (input: string) => Effect.gen(function* () {
    // Parse the URL
    let url: URL
    try {
      url = new URL(input)
    } catch {
      return yield* Effect.fail(new InvalidUrlError({
        input,
    ...
    if (!parser) {
      return yield* Effect.fail(new UnsupportedServiceError({
        url: input,
