...
import { NetworkError, HandleNotFoundError, InvalidHandleError, RateLimitError, InvalidDidDocumentError, DidResolutionError, type ResolverError } from '@/errors/resolver-errors';
...
const retrySchedule = pipe(Schedule.exponential('100 millis'), Schedule.jittered, Schedule.compose(Schedule.recurs(3)));
      ...
      if (didDoc.alsoKnownAs) {
        for (const aka of didDoc.alsoKnownAs) {
          if (aka.startsWith('at://')) {
            ...
            const validatedHandle = yield* S.decodeUnknown(Handle)(handle).pipe(Effect.option);
