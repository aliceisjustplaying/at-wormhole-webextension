...
// Service tag
export class Normalizer extends Context.Tag('Normalizer')<Normalizer, NormalizerService>() {}
...
const buildBskyAppPath = (identifier: string, contentType: string, rkey?: string): string => {
  ...
  normalize: (input: string) =>
    Effect.gen(function* () {
      ...
      const info = yield* createTransformInfo({
        ...
        bskyAppPath,
        inputType:
          inputType === 'fragment' ? 'handle'
          : inputType === 'at-uri' ? 'url'
          : inputType,
        contentType,
      }).pipe(
        Effect.mapError(
