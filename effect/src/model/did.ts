...
const DidPlcSchema = S.String.pipe(
  S.pattern(/^did:plc:[a-z0-9]{24}$/),
  S.brand("DidPlc")
)

// Create branded PLC examples
const plcExamples = [
  S.decodeSync(DidPlcSchema)("did:plc:z72i7hdynmk6r22z27h6tvur"),
  S.decodeSync(DidPlcSchema)("did:plc:abcdef1234567890abcdef12")
] as const
...
const DidWebSchema = S.String.pipe(
  S.pattern(/^did:web:[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/),
  S.brand("DidWeb")
)

// Create branded Web examples
const webExamples = [
  S.decodeSync(DidWebSchema)("did:web:example.com"),
  S.decodeSync(DidWebSchema)("did:web:alice.example.org")
] as const
