...
class Calculator extends Context.Tag('Calculator')<Calculator, CalculatorService>() {}

describe('Layer.succeed explained', () => {
  it.effect('shows what Layer.succeed actually does', () =>
    Effect.gen(function* () {
      ...
      const result = yield* program.pipe(Effect.provide(CalculatorLayer));

      expect(result).toBe(5);
    }),
  );

  it.effect('shows the difference between Layer types', () =>
    Effect.gen(function* () {
      ...
      const ScopedLayer = Layer.scoped(
        ...
        Effect.gen(function* () {
          ...
          // Register cleanup
          yield* Effect.addFinalizer(() => Effect.log('Calculator shutting down'));
      ...
      const r1 = yield* Effect.provide(program, SimpleLayer);
      expect(r1).toBe(2);
    }),
  );

  it.effect('shows why layers are useful', () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.provide(program, AppLayer);
