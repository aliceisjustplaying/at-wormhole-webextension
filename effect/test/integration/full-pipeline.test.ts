...
import type { ResolverError } from '@/errors/resolver-errors';
...
describe('Full Transform Pipeline', () => {
  ...
  const AppLayer = Layer.mergeAll(Layer.succeed(UrlParser, UrlParserLive), Layer.succeed(Normalizer, NormalizerLive), Layer.succeed(Resolver, ResolverLive));

  const transformUrl = (url: string): Effect.Effect<TransformInfo, UrlParseError | NormalizeError | ResolverError, UrlParser | Normalizer | Resolver> =>
    Effect.gen(function* () {
  ...
  it.effect('should transform bsky.app URL with handle resolution', () =>
    Effect.gen(function* () {
      ...
      const result = yield* transformUrl('https://bsky.app/profile/alice.bsky.social/post/3kt7p4fzxhh2c').pipe(Effect.provide(AppLayer));
  ...
  it.effect('should handle URLs that already have DIDs', () =>
    Effect.gen(function* () {
      // No fetch should occur for DID-based URLs

      const result = yield* transformUrl('https://bsky.app/profile/did:plc:z72i7hdynmk6r22z27h6tvur/post/3kt7p4fzxhh2c').pipe(Effect.provide(AppLayer));
  ...
  it.effect('should gracefully handle resolution failures', () =>
    Effect.gen(function* () {
      ...
      const result = yield* transformUrl('https://bsky.app/profile/unknown.handle/post/3kt7p4fzxhh2c').pipe(Effect.provide(AppLayer));
  ...
  it.effect('should transform skythread URL with DID resolution', () =>
    Effect.gen(function* () {
      const result = yield* transformUrl('https://blue.mackuba.eu/skythread/?author=did:plc:2p6idfgjfe3easltiwmnofw6&post=3lpjntj43rs23').pipe(Effect.provide(AppLayer));
  ...
  it.effect('should handle did:web URLs', () =>
    Effect.gen(function* () {
      ...
      const result = yield* transformUrl('https://bsky.app/profile/did:web:example.com').pipe(Effect.provide(AppLayer));
  ...
  describe('error propagation', () => {
    it.effect('should fail on unsupported service', () =>
      Effect.gen(function* () {
        const result = yield* transformUrl('https://unsupported.com/something').pipe(Effect.provide(AppLayer), Effect.either);
    ...
    it.effect('should fail on invalid URL format', () =>
      Effect.gen(function* () {
        const result = yield* transformUrl('not a url').pipe(Effect.provide(AppLayer), Effect.either);
